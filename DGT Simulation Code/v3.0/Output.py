...
#import pandas as pd
########################################################################################################
def output_time_data(exp,ic, t, Drunks):
...
########################################################################################################
# def output_agent_data(exp,ic, Drunks, t = 0):
# 	alphas = []
# 	cooperators = []
# 	strategies = []
# 	payoffs = []
...
# 	filename = Par.dirname + "%i_%i_agent_data_%i.csv" % (exp, ic, t)
...
# 	df.to_csv(filename)
########################################################################################################
def output_agent_data_np(exp,ic, Drunks, t = 0):
	alphas = []
	cooperators = []
	strategies = []
	payoffs = []
	...
	filename = Par.dirname + "%i_%i_agent_data_%i.npy" % (exp, ic, t)
	#columns = ['alphas', 'strategies', 'payoffs', 'beers', 'cooperator_alphas' ]
	arr = np.array([alphas, strategies, payoffs, cooperator_alphas])
	np.save(filename, arr)

########################################################################################################
def output_agent_data_averages(exp,ic, Drunks, t = 1):
	alphas = []
	cooperators = []
	strategies = []
	payoffs = []
	...
	filename = Par.dirname + "%i_average_agent_data.npy" % (exp)
	average_alpha = np.mean(alphas)
	average_payoff = np.mean(payoffs)
	average_x = np.mean(strategies)
	if ic == 0 and t == 0:
		arr = np.array([ic,t,average_alpha, average_payoff, average_x])
	else:
		arr = np.load(filename)
		#print arr
		new_data = np.array([ic,t, average_alpha, average_payoff, average_x])
		arr = np.vstack((arr, new_data))
		#print arr
		#raw_input("Enter")
	np.save(filename, arr)
########################################################################################################
def output_initial_data(exp,ic, Drunks):
	alphas = []
	cooperators = []
	strategies = []
	payoffs = []
	...
	# filename = Par.dirname + "%i_%i_initial_data.csv" %(exp, ic)
	# columns = ['alphas', 'strategies', 'payoffs', 'beers', 'cooperator_alphas' ]
	# df = pd.DataFrame([alphas, strategies, payoffs, beers, cooperator_alphas], index = columns)
...
#########################################################################################################
def output_network(Bar_network, Drunks, exp, ic, t = 0, draw = False):
	...
	if Par.graph_type != 'Groups':
		...
		fname = Par.dirname +'%i_%i_bar_network_%i.csv' % (exp,ic, t)
