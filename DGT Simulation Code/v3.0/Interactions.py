...
###############################################################################################################
def alpha_update_old(alpha_0, Beers):
...
###############################################################################################################
def alpha_update_new(alpha_0, cooperate):
	''' Returns the new alpha value of a player given their current alpha and whether their partner cooperated'''
	if cooperate == True:
		new_alpha = alpha_0 + Par.kappa*alpha_0*(1-alpha_0)
	else:
		new_alpha = alpha_0 - Par.kappa*alpha_0*(1-alpha_0)
...
def play_game(match, Drunks):
	...
	if Player_One_Sober == True: 
		Player_One_Payoff = sober_game(Player_One_Offers, Player_Two_Offers)
	else:
		Player_One_Payoff = drunk_game(Player_One_Offers, Player_Two_Offers)

	# Player Two payoff/drinks
	if Player_Two_Sober == True:
		Player_Two_Payoff = sober_game(Player_Two_Offers, Player_One_Offers)
	else:
		Player_Two_Payoff = drunk_game(Player_Two_Offers, Player_One_Offers)
	...
	# New Alphas
	Alpha_1 = alpha_update_new(player1.alpha, player2.cooperator)
	player1.alpha = Alpha_1
	#print 'Alpha_1', Alpha_1

	Alpha_2 = alpha_update_new(player2.alpha, player1.cooperator)
	player2.alpha = Alpha_2
...
##############################################################################################################
def fermi_rule_update_asynchronous( Bar_network, Drunks):

	from Parameters import beta
	# Pick one player
	probabilities = []
	payoffs = []
	players = Bar_network.nodes()
	for player_ID in players:
		...
		for drunk_person in Drunks:
			drunk_person.payoff = 0.0
##############################################################################################################
def fermi_rule_update_synchronous( Bar_network, Drunks):

	from Parameters import beta
	# Pick one player
	payoffs = []
	players = Bar_network.nodes()
	new_strat_profiles = [None]*len(players)
	for player_ID in players:

		player_edges = nx.edges(Bar_network, player_ID)
		player = Drunks[player_ID]
		# Make sure you've actually got a player in the network
		if player_edges == []:
			new_strat_profiles[player_ID] = Drunks[player_ID].strat
			continue
		...
		change_strat = False
		dice_roll = random.random()
		if dice_roll <= p_ij: # Change
			change_strat = True
			new_strat_ID = other_player
			new_strat_profiles[player_ID] = Drunks[new_strat_ID].strat
		else: # Stay
			new_strat_profiles[player_ID] = Drunks[player_ID].strat
			
		# Clear payoffs from calculation	
		for drunk_person in Drunks:
			drunk_person.payoff = 0.0
	
	for player_ID in players:
		if new_strat_profiles[player_ID] == None:
			print player_ID, Drunks[player_ID].strat
			raw_input("enter")
		Drunks[player_ID].strat = new_strat_profiles[player_ID]
